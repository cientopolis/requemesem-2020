...
class SentencesLoader:
    def __is_known_token(self, table, token):
        atributes = [
            token.dep_,  # must have same order
            token.pos_,
        ]  # as the table
    ...
    def __keep_token(self, token):
        return (not self.__is_blacklisted(token)) or self.__is_whitelisted(
            token
        )
    ...
    def __find_pos_(
        self, doc, pos_
    ):  # can be any form of token iterable, not just a doc
    ...
    def __get_kernel_sentence(self, doc):
        return self.__add_formatting(
            self.__doc_as_strings(self.__delete_until_pos(doc, "DET"))
        )
    ...
    def __get_sentence_from_parenthesis(self, doc):
        result = (
            self.__get_previous_word(doc)
            + " es "
            + doc.text[doc.text.find("(") + 1 : doc.text.find(")")]
        )
    ...
    def __parse_conj_y(self, doc, conjIndex):
        ...
        if verbIndex == -1:
            ...
            lastVerbIndex = len(before) - self.__find_pos_(
                list(reversed(before)), "AUX"
            )  # el AUX es "varían"
            result.append(
                doc[:lastVerbIndex].text + " " + doc[conjIndex + 1 :].text
            )
        else:
    ...
    def __parse_conj_como(self, doc, conjIndex):
        ...
        lastPropnIndex = len(before) - self.__find_pos_(
            list(reversed(before)), "PROPN"
        )
        result.append(
            doc[:lastPropnIndex].text + " " + doc[conjIndex + 1 :].text
        )
        return result
    ...
    def __has_adj(self, sentence):
        return self.__find_pos_(
            sentence[self.__find_pos_(sentence, "VERB") :], "ADJ" != -1
        )
        ...
        for st in sentences:
            if self.__has_noun(st):
                ...
                if previous_noun:
                    sentences.remove(st)
                    explicit_st = (
                        st[: self.__find_dep_(st, "nsubj")].text
                        + " "
                        + previous_noun
                        + " "
    ...
    def __is_definite_article(self, token):
        ...
        return (
            token.tag_[token.tag_.find("=") + 1 : token.tag_.find("|")]
            == "Def"
        )

    def __swap_object(self, doc):
        first, second = self.__find_two_dep_(
            doc[self.__find_pos_(doc, "VERB") + 1 :], "obj"
        )
    ...
    def __first_blacklist(self, doc):
        ...
        blacklist = [["cop"], []]  # dep  # pos

        result = list(
            map(
                lambda token: token.text,
                filter(
                    lambda token: not self.__is_known_token(blacklist, token),
                    doc,
                ),
            )
        )
