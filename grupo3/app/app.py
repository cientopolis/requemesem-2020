    ...
    if request.method == "POST":
        if request.form["nombre"] and request.form["email"]:
            ...
            try:
                ...
                query = (
                    "Insert into usuarios(mail,nombreYApellido)values('"
                    + request.form["email"]
                    + "','"
                    + request.form["nombre"]
                    + "')"
    ...
    if request.method == "POST":
        ...
        if "file" in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                # El decode("UTF-8") es porque lo trae como ascii,
                # por lo que las tildes y otros simbolos se ven mal
                texto_default = str(file.read().decode("UTF-8"))
                try:
                    connection = mysql.connector.connect(**config)
                    cursor = connection.cursor(buffered=True)
                    query = (
                        "Insert into archivos(texto,usuario)values('"
                        + texto_default
                        + "','"
                        + session.get("email")
                        + "')"
                ...
                return render_template(
                    "home.html", texto_default=texto_default
                )
            else:
...
def procesar_texto_basico(_texto_basico):
    ...
    try:
        ...
        processedSentences = (
            "Hubo un error en la aplicacion, por favor intente de nuevo"
        )
...
def mejorado():
    if request.method == "POST":
        if "siguiente" in request.form:
            ...
            return render_template(
                "mejorado.html", lista_mejorado=lista_texto_mejorado
            )
