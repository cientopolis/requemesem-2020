...
class Tool:
    ...
    def __buscarObjetos(self, doc):
        ...
        if od == "":
            ...
            for i in range(posVerbo, len(doc)):
                if (
                    (doc[i].pos_ == "NOUN" and doc[i].dep_ == "nmod")
                    or (
                        doc[i].dep_ == "obj"
                        and (
                            doc[i].pos_ == "NOUN"
                    ...
                    if (
                        ind == 0
                    ):  # el "ind" se está usando para marcar si había o no modificador, si hay queda en 1
                        od = doc[i].text
                        if (
                            doc[i + 1].pos_ == "ADP"
                            and doc[i + 1].text != "al"
                            and doc[i + 1].text != "a"
                            and doc[i + 1].text != "para"
        ...
        if doc[posOD].text != ".":
            for i in range(posOD, len(doc)):
                ...
                if (
                    ...
                    or doc[i].text == "a"
                    or doc[i].text == "al"
                ):  # El OI está encabezado por a / para
                    posOI = i
                    break
            if posOI != None:
    ...
    def __get_relation(self, sent):
        ...
        if tieneOI:
            ...
            pos = 0
            while pos < len(doc):
                if (doc[pos].pos_ != "DET") or (
                    doc[pos].pos_ == "DET" and doc[pos].text == "un"
                ):
                    rel = rel + " " + doc[pos].text
                pos += 1
...
class UMLProcessing:
    ...
    def __buscar_subclases(self, lemma, lista_clases, dicLemma):
        subclases = []
        for clase in lista_clases:
        ...
        for _, row in df.iterrows():
            ...
            if existe == None:
                ...
                if self.__es_variable(row["relacion"]):
                    ...
                    rel = self.__determinarRelacion(
                        row["relacion"], row["Entidad2"]
                    )
                    nueva.addMetodo(rel)
    ...
    def __procesamientoDOS(self, lista_clases):
        ...
        for clase in lista_clases:
            if (
                dicLemma[clase.getNombre()] == clase.getNombre()
            ):  # procesara las de mismo lema y nombre
                clase.setSubclases(
                    self.__buscar_subclases(
                        dicLemma[clase.getNombre()],
                        lista_clases,
                        dicLemma,
                    )
                )
        ...
        for clase in lista_clases:
            if (
                not self.__es_subclase(clase)
                and not clase in self.__lista_final
            ):
    ...
    def __generateUMLFile(self):
        ...
        for clase in self.__lista_final:
            ...
            for subclase in clase.getSubclases():
                ...
                file.write(
                    'class "' + subclase.getNombre() + '" {' + os.linesep
                )
