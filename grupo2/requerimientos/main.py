...
def build_triples(sentence_list, dataset):
    for sentence in sentence_list:
        # sujeto-predicado-objeto
        triples = (
            get_entities(sentence)[0],
            getRelation(sentence),
            get_entities(sentence)[1],
        )
...
def get_all_entities(sentence_list):
    return set(chain.from_iterable(map(get_entities, sentence_list)))
...
def create_graph(sentence_list):
